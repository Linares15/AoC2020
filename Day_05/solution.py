boardingPasses = []
with open("Day_05/input.txt") as puzzleInput:
...
def computeSeatId(row, column):
    seatId = row * 8 + column
    return seatId

def solvePart1(boardingPasses):
    maxSeatId = 0
    for boardingPass in boardingPasses:
        row = computeRow(boardingPass)
        column = computeColumn(boardingPass)
        seatId = computeSeatId(row, column)
        if seatId > maxSeatId:
            maxSeatId = seatId
    return maxSeatId
...
print(f"Part 1: {solvePart1(boardingPasses)}")
print(f"Part 2: {solvePart2(boardingPasses)}")
