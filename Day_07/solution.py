...
class Bag:
    ...
    def add(self, count, color):
        self.Contents.append((count, color))

bags = {}
with open("Day_07/input.txt") as puzzleInput:
    for line in puzzleInput:
        ...
        while (len(parts) != 0):
            if parts[0] == "no":
                ...
                bag.add(int(parts[0]), f"{parts[1]} {parts[2]}")
                parts = parts[4:]
...
def containsShinyGold(bag, allBags):
    for (count, color) in bag.Contents:
        if color == "shiny gold":
            return True
        if containsShinyGold(allBags[color], allBags):
            return True
    return False
...
print(f"Part 1: {solvePart1(bags)}")
...
def countNestedBags(bag, allBags):
    totalBags = 1
    for (count, color) in bag.Contents:
        totalBags = totalBags + (count * countNestedBags(allBags[color], allBags))
    return totalBags


def solvePart2(bags):
    goldBag = bags["shiny gold"]
    return countNestedBags(goldBag, bags) -1

print(f"Part 2: {solvePart2(bags)}")
