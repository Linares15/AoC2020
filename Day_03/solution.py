grid = []
with open("Day_03/input.txt") as txt:
    for line in txt:
        row = []
        for mark in line[0:-1]:
            row.append(mark)
        grid.append(row)

# This function is given because you haven't learned modular arithmetic yet.
def isTree(grid, x, y):
    return grid[y][x % len(grid[y])] == "#"
...
def countTrees(grid, slopeX, slopeY):
    x = 0
    y = 0
    trees = 0
    while y < len(grid):
