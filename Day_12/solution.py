...
actions = []
with open("Day_12/input.txt") as puzzleInput:
    for line in puzzleInput:
        match = re.match("([NSEWLRF])(\d+)", line)
        action = Action(match[1], int(match[2]))
...
def solvePart1(actions):
    ...
    for action in actions:
        if action.direction == "N":
            ...
            rotation = 0
            while rotation < action.amount:
                rotation = rotation + 90
                facing = leftRotation(facing)
        elif action.direction == "R":
            rotation = 0
            while rotation < action.amount:
                rotation = rotation + 90
                facing = rightRotation(facing)
        elif action.direction == "F":
            if facing == "N":
                y = y + action.amount
            if facing == "S":
                y = y - action.amount
...
print(f"Part 1: {solvePart1(actions)}")
...
def solvePart2(actions):
    pass

print(f"Part 2: {solvePart2(actions)}")
