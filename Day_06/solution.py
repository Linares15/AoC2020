groups = []
with open("Day_06/input.txt") as puzzleInput:
    members = []
    for answers in puzzleInput:
        if answers == "\n":
            groups.append(members)
            members = []
        else:
            members.append(answers.replace("\n", ""))
    groups.append(members) # Cleanup
...
def solvePart1(groups):
    ...
    for group in groups:
        ...
        total = total + len(uniqueAnswers)
    return total

print(f"Part 1: {solvePart1(groups)}")
...
def solvePart2(groups):
    total = 0
    for group in groups:
        actualAnswer = list(group[0])
        for member in group:
            for answer in reversed(actualAnswer):
                if answer not in member:
                    actualAnswer.remove(answer)
        total = total + len(actualAnswer)
    return total

print(f"Part 2: {solvePart2(groups)}")
